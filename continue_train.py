...
## up to here it was the same as the standard training script.
## now we load the already trained model
...
# list all saved model files (for all epochs)
files = sorted(glob.glob(modelpath+'/'+paramstr+'???_*.h5'))
# get the last one (highest epoch). This is the last one on the sorted list
modelfile = files[-1]
# extract the epoch number
last_epoch = int(modelfile[-13:-10])
...
# the loaded net has twice the amount of variables. this is because in the net, two passes are made
# through the same part of the net. since we saved in .h5 format, this information is not preserved,
# and the loaded net has two versions of each variable. since they are identical, we can extract the first versions,
# and the set the variables in the untrained net to these values.
# then we have a version of the trained net that we can train further.

# note that this works only if the main trainign script stopped in the first 100 epochs. Due to a bug in
# # the main training script the continuation (with early stopping) starts numbering the filenames with 0 again.

# check whether the variables are really duplicates
n_net_vars = len(net.trainable_variables)
assert(2*n_net_vars == len(net_loaded.trainable_variables))

for i in range(n_net_vars):
    np.testing.assert_array_equal(net_loaded.trainable_variables[i].numpy(),
                                  net_loaded.trainable_variables[n_net_vars+i].numpy())


# now set the weights of the freshly initialized net to the trained ones (to the first half of the loaded ones)
net.set_weights(net_loaded.get_weights()[:n_net_vars])

# check again that this worked
for i in range(n_net_vars):
    np.testing.assert_array_equal(net.trainable_variables[i].numpy(),
                                  net_loaded.trainable_variables[i].numpy())
...
hist2 = net.fit(dataset_train, validation_data=dataset_valid,
               initial_epoch=n_epochs_no_earlystop,
               steps_per_epoch=n_train//batch_size,
               validation_steps=n_valid // batch_size, epochs=n_epochs_no_earlystop+ n_epochs_additional,
               callbacks=[
                   tf.keras.callbacks.ModelCheckpoint(modelpath + '/' + paramstr + '{epoch:03d}_{val_loss:.4f}.h5'),
...
pickle.dump([hist1.history,hist2.history], open(f'{histpath}/{paramstr}.hist_continued.pkl', 'wb'))
