...
for nettype in ('unet_base', 'unet_sphereconv', 'unet_sphereconv_hemconv_shared',
    ...
    for fcday in range(1,11):
        dates_valid = pd.read_csv(f'data/bust_dates_SH_{nettype}_fcday{fcday}.csv', index_col=0)
        ...
        plt.tight_layout()
        plt.savefig(f'bust_anomaly_z500_SH_{nettype}_fcday{fcday}.png', dpi=300, bbox='tight')
        plt.savefig(f'bust_anomaly_z500_SH_{nettype}_fcday{fcday}.svg')
        plt.close('all')