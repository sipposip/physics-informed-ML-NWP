...
for nettype in ('unet_base', 'unet_sphereconv', 'unet_sphereconv_hemconv_shared',
    ...
    for fcday in range(1,11):
        ...
        sns.despine()
        plt.tight_layout()
        plt.savefig(f'plots/barplot_forecast_busts_{nettype}_fcday{fcday}.svg')
        ...
        sns.despine()
        plt.tight_layout()
        plt.savefig(f'plots/ycycleforecast_busts_{nettype}_fcday{fcday}.svg')

        pd.Series(list(dates_all)).to_csv(f'data/bust_dates_{nettype}_fcday{fcday}.csv')
